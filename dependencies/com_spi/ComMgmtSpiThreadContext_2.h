...
#ifndef ComMgmtSpiThreadContext_2_h
#define ComMgmtSpiThreadContext_2_h

#include <stdint.h>                  /* uint64_t               */
#include "ComMgmtSpiCommon.h"        /* ComReturnT             */
#include "ComMgmtSpiInterface_1.h"   /* ComMgmtSpiInterface_1T */


/**
 * @file ComMgmtSpiThreadContext_2.h
 *
 ...
 * function returns \c ComFailure, can be complemented with an error message
 ...
 *
 * It is recommended that the one initiating a call chain takes responsibility
 * for clearing the texts in the thread context before initiating a call chain
 * the can return an error code, or other indication that can be include
 * text that shall be logged or displayed to a human.
 *
 * For the NBI category it is the NBI that has the responsibility to clear and
 * use any messages returned in the thread context.
 *
 * In general, the one writing text into the thread context does not know
 * when this information can be cleared. For example, when calling the Transaction
 * master validate, all validators will be called, even if one or more fails,
 * in order to collect information from all validators, to be displayed to the user.
 * Therefore, the clearing of the thread context must be the responsibility of
 * the top initiator of a call chain.
 */
 ...
 *
 * It is recommended that messages contains enough information and context to be
 * understandable by an operator.
 *
 * The enum defines a monotonically increasing number of values
...
typedef enum ThreadContextMsgCategory_2 {
    ...
    /**
     * The log category is to be used for messages suitable for internal
    ...
    /**
     * The NBI category is to be used for messages suitable for external
 ...
 * functions will be handled in the context of the current thread and will
 ...
 */
typedef struct ComMgmtSpiThreadContext_2 {
     ...
     *
     * Generally, information in a message must not depend on information in
     ...
     * @return ComOk if message was successfully added.
     */
    ComReturnT (*addMessage) (ThreadContextMsgCategory_2T category, const char* message);
     ...
     * message iterating functions of this SPI. The contents of the
     ...
     * @return ComOk if an iterator was successfully created. If ComOk is
     *         returned, \p iteratorHandle refers to the new iterator and can be
     *         used as parameter to message handling functions.
     */
    ComReturnT (*newMessageIterator) (ThreadContextMsgCategory_2T category, ThreadContextHandle_2T* handle);

    /**
     * Gets the next message in the iteration referred to by the provided
     * handle. After a message has been returned by this function, the
     * iterator will be advanced and refer to the following message.
     *
     * When the iteration is complete as indicated by this function,
     * the iterator handle can no longer be used as parameter to iterate
     * functions. When this function indicates end of iteration,
     ...
     * @param[out] message If ComOk is returned, \p message points to the next
     *             current message in the iteration or NULL if the iteration has
     *             reached the end.
     * @return ComOk if \p next message could be returned successfully.
     */
    ComReturnT (*nextMessage)  (ThreadContextHandle_2T handle, const char** message);
     ...
     * @return ComOk if all \p category messages could be cleared successfully.
     */
    ComReturnT (*clearMessages) (ThreadContextMsgCategory_2T category);
     ...
     *                   call was successful.
     * @return ComOk if the message count could be returned successfully.
     */
    ComReturnT (*messageCount)  (ThreadContextMsgCategory_2T category, uint32_t* count);
     ...
     *
     * When the messages retrieved using this function are no longer needed,
     * the associated handle must be released by calling \c releaseHandle().
     ...
     * @param[out] messages If successful, \p messages will be set to a null
     *             terminated array of all message strings in \p category.
     * @return ComOk if messages was successfully retrieved.
     */
    ComReturnT (*getMessages)  (ThreadContextMsgCategory_2T category, ThreadContextHandle_2T* msgHandle, const char*** messages);

    /**
     * Release a ThreadContext handle. This function must be called when a
     * handle and its associated data are no longer needed. After the call,
     ...
     * @return ComOk if handle was released successfully.
     */
    ComReturnT (*releaseHandle) (ThreadContextHandle_2T handle);

} ComMgmtSpiThreadContext_2T;
