 ...
 * match the event type and the filter. If the consumer unregisters from
...
typedef struct ComOamSpiEventProducer_1 {
    ...
    /**
     * doneWithValue, Event router calls this function to notify the Producer that
...
typedef struct ComOamSpiEventRouter_1 {
     ...
     *
     * @param filter [in] the same filter instance that was used by the addSubscription call.
     *
     * @return ComOk, or @n
     * ComInvalidArgument if handle is not registered or a parameter is null, or @n
    ...
    /**
     * Remove an event type registered in the Event Router by the producer.
     *
     * @param handle [in] identifier of the producer. This value is returned when the producer
     *					  register its interface in the Event Router.
     *
